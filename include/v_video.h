...
// Cached named-patch lookup for repeatedly drawn menu/finale graphics.
int V_GetPatchNumCached(const char* name);


// V_DrawNamePatch - Draws the patch from lump "name"
#define V_DrawNamePatch(x,y,s,n,t,f) V_DrawNumPatch(x,y,s,V_GetPatchNumCached(n),t,f)
 ...
 * this code.
 */
#define V_NamePatchWidth(name) R_NumPatchWidth(V_GetPatchNumCached(name))
#define V_NamePatchHeight(name) R_NumPatchHeight(V_GetPatchNumCached(name))
