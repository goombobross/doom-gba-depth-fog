...
/*
 * Exact distance/speed division for built-in Doom projectiles.
 *
 * Both operands are 16.16 fixed point and all built-in missile speeds are
 * integral values: 10, 15, 20 or 25 map units per tic.  Dropping the
 * fractional distance before dividing is exact for an integral fixed-point
 * divisor.  The bounded multiply/shift quotients below are exact for every
 * positive fixed_t distance, avoiding ARM's software integer divider on the
 * common path.  The fallback preserves behavior for any unusual custom speed.
 */
inline static CONSTFUNC int FixedDistanceToTicks(fixed_t distance, fixed_t speed)
{
    const unsigned int uspeed = (unsigned int)speed;

    if (distance >= 0 && speed > 0 &&
        (uspeed & (FRACUNIT - 1u)) == 0u)
    {
        const unsigned int units = (unsigned int)distance >> FRACBITS;

        switch (uspeed >> FRACBITS)
        {
            case 10u: return (int)((units * 26215u) >> 18);
            case 15u: return (int)((units * 34953u) >> 19);
            case 20u: return (int)((units * 26215u) >> 19);
            case 25u: return (int)((units *  5243u) >> 17);
        }
    }
