...
void M_ChangeDepthFog(int choice);
...
void M_DrawOptions(void)
{
  ...
  // Menu patches use Doom's 320x200 virtual coordinates, while M_WriteText
  // draws directly in native 240x160 GBA pixels. Convert the custom label so
  // it occupies the same row as the menu cursor, between Gamma and Sound.
  const int fogTextX = (OptionsDef.x * 240) / 320;
  const int fogTextY = ((OptionsDef.y + LINEHEIGHT*depthfog) * SCREENHEIGHT) / 200 + 2;

  M_WriteText(fogTextX, fogTextY, "DEPTH FOG");

  // Reuse Doom's existing ON/OFF menu graphics for a consistent appearance.
  V_DrawNamePatch(OptionsDef.x + 150, OptionsDef.y+LINEHEIGHT*depthfog, 0,
      _g->depthFog ? "M_MSGON" : "M_MSGOFF", CR_DEFAULT, VPT_STRETCH);
}
...
{
    choice = 0;
    _g->depthFog = 1 - _g->depthFog;

    _g->player.message = _g->depthFog ? "DEPTH FOG ON" : "DEPTH FOG OFF";
    _g->message_dontfuckwithme = true;
