...
// The draw path uses -O2, matching asie's fast-IWRAM strategy.
// A post-link budget guard prevents an unsafe ROM from being emitted.
...
// Exact reciprocal-table normalization without the variable-length while loop
// in FixedReciprocal(). ARM7TDMI has no CLZ instruction, so five predictable
// range tests are cheaper for the scale and geometry values used here.
inline static CONSTFUNC fixed_t R_FastReciprocal(fixed_t v)
{
    unsigned int val = v < 0 ? -v : v;
    unsigned int shift = 0;

    if (val > 0x01000000u) { val >>= 8; shift += 8; }
    if (val > 0x00100000u) { val >>= 4; shift += 4; }
    if (val > 0x00040000u) { val >>= 2; shift += 2; }
    if (val > 0x00020000u) { val >>= 1; shift += 1; }
    if (val > 0x00010000u) { val >>= 1; shift += 1; }

    fixed_t result = reciprocalTable[val] >> shift;
    return v < 0 ? -result : result;
}

inline static CONSTFUNC fixed_t R_FastApproxDiv(fixed_t a, fixed_t b)
{
    return FixedMul(a, R_FastReciprocal(b));
}
...
static CONSTFUNC int SlopeDiv(unsigned num, unsigned den)
{
    ...
    const unsigned int ans = R_FastApproxDiv(num << 3, den) >> FRACBITS;
...
CONSTFUNC angle_t R_PointToAngle2(const fixed_t vx, const fixed_t vy, fixed_t x, fixed_t y)
{
    ...
    const boolean xMajor = absx > absy;
    angle_t base;

    if (xMajor)
        base = tantoangle[SlopeDiv(absy, absx)];
    else
        base = ANG90 - 1 - tantoangle[SlopeDiv(absx, absy)];

    if (x >= 0)
    {
        if (y < 0)
            base = xMajor ? -base : -1-base;
    }
    else if (y >= 0)
        base = ANG180 - 1 - base;
    else
...
static CONSTFUNC fixed_t R_PointToDist(fixed_t x, fixed_t y)
{
    ...
    return R_FastApproxDiv(dx, finesine[(tantoangle[R_FastApproxDiv(dy,dx) >> DBITS] + ANG90) >> ANGLETOFINESHIFT]);
}

// Apply sector orientation and muzzle-flash brightness once. Wall and masked
// loops can reuse the adjusted result instead of repeating this work per band.
inline static int R_AdjustLightLevel(int lightlevel)
{
    if (curline)
    {
...
inline static int R_ColourMapNumberAdjusted(int lightlevel, fixed_t scale, boolean depthFog)
{
    ...
    if (depthFog)
    {
        ...
        {
            const unsigned int s = (unsigned int)scale;
            depth = (s + (s >> 2) + (s >> 4) + (s >> 6)) >> LIGHTSCALESHIFT;
        }

        cm += 20 - depth;
    }

    if ((unsigned int)cm >= NUMCOLORMAPS)
        cm = cm < 0 ? 0 : NUMCOLORMAPS-1;
...
// adjusted helper directly so classic lighting has no per-column fog overhead.
static const lighttable_t* R_ColourMap(int lightlevel, fixed_t scale)
{
    return R_ColourMapAdjusted(R_AdjustLightLevel(lightlevel), scale, _g->depthFog);
}

// Load one selected colormap into the existing 256-byte IWRAM cache.
inline static const lighttable_t* R_LoadColorMapPointer(const lighttable_t* lm)
{
...
static const lighttable_t* R_LoadColorMap(int lightlevel, fixed_t scale)
{
    return R_LoadColorMapPointer(R_ColourMap(lightlevel, scale));
}
...
#ifdef GBA
/*
 * ARM7TDMI vertical column mapper with adaptive magnification reduction.
 *
 * Normal-scale columns retain the exact ABI-safe two-pixel pipeline. Columns
 * magnified by at least 2x sample once per two output rows; columns magnified
 * by at least 4x sample once per four output rows. At those scales the source
 * texel naturally covers multiple screen rows, so this cuts expensive source
 * and colormap reads while keeping the column bounds and framebuffer writes
 * exact. The close-up paths trade a very small amount of vertical sampling
 * precision for substantially less work.
 *
 * Compact remainder loops and 8-row close-up blocks keep this path inside
 * the project's strict IWRAM-to-stack safety budget.
 *
 * Register allocation deliberately uses only r4-r7 from the callee-saved set,
 * reducing stack traffic on every column call. r8-r11 remain untouched.
 *
 * draw_column_vars_t offsets: x=0, yl=4, yh=8, iscale=12,
 * texturemid=16, source=20, colormap=24.
 */
...
static void R_DrawColumn(const draw_column_vars_t *dcvars)
{
    __asm__ volatile(
        ...
        "rsb r7, r12, r12, lsl #4\n\t"
        "add r2, r2, r7, lsl #4\n\t"      /* + yl*240 bytes */
        "ldr r7, [r0, #0]\n\t"
        "add r2, r2, r7, lsl #1\n\t"      /* + x*2 */
        "ldr r0, [r0, #20]\n\t"          /* source; dcvars no longer needed */

        /* >=4x magnification: one lookup for four output rows. */
        "cmp r4, #0x00800000\n\t"         /* iscale <= FRACUNIT/4 */
        "bls 30f\n\t"
        /* >=2x magnification: one lookup for two output rows. */
        "cmp r4, #0x01000000\n\t"         /* iscale <= FRACUNIT/2 */
        "bls 20f\n\t"

        /* Exact normal-scale path: 16 output rows per block. */
        "and r6, r5, #15\n\t"
        "movs r5, r5, lsr #4\n\t"
        "beq 11f\n\t"
        "10:\n\t"
        "ldrb r7, [r0, r3, lsr #25]\n\t"
        "add r3, r3, r4\n\t"
        "ldrb r12, [r0, r3, lsr #25]\n\t"
        "add r3, r3, r4\n\t"
        "ldrb r7, [r1, r7]\n\t"
        "ldrb r12, [r1, r12]\n\t"
        "strb r7, [r2, #0]\n\t"
        ...
        /* Two-row sampling path, compact 8-row blocks. */
        "20:\n\t"
        "mov r4, r4, lsl #1\n\t"          /* 2*fracstep */
        "and r6, r5, #7\n\t"
        "movs r5, r5, lsr #3\n\t"
        ...
        /* Four-row sampling path, compact 8-row blocks. */
        "30:\n\t"
        "mov r4, r4, lsl #2\n\t"          /* 4*fracstep */
        "and r6, r5, #7\n\t"
        "movs r5, r5, lsr #3\n\t"
...
static void R_RenderMaskedSegRange(const drawseg_t *ds, int x1, int x2)
{
    ...
    const boolean depthFog = _g->depthFog;
    const int adjustedLight = R_AdjustLightLevel(rw_lightlevel);

    // Classic lighting is constant for this entire masked segment.
    if (!depthFog)
        dcvars.colormap = R_LoadColorMapPointer(R_ColourMapAdjusted(adjustedLight, 0, false));
    ...
    for (dcvars.x = x1 ; dcvars.x <= x2 ; dcvars.x++, spryscale += rw_scalestep)
    {
        ...
        {
            if (depthFog)
                dcvars.colormap = R_LoadColorMapPointer(R_ColourMapAdjusted(adjustedLight, spryscale, true));

            sprtopscreen = centeryfrac - FixedMul(dcvars.texturemid, spryscale);

            dcvars.iscale = R_FastReciprocal((unsigned)spryscale);
...
static void R_DrawPSprite (pspdef_t *psp, int lightlevel)
{
    ...
    if (_g->player.powers[pw_invisibility] > 4*32 || _g->player.powers[pw_invisibility] & 8)
        ...
        vis->colormap = R_LoadColorMap(lightlevel, FRACUNIT*SCREENWIDTH/128);  // preserve near-field weapon light
...
static void R_ProjectSprite (mobj_t* thing, int lightlevel)
{
    ...
    const fixed_t iscale = R_FastReciprocal(xscale);
    ...
    {      // diminished light
        vis->colormap = R_ColourMap(lightlevel, vis->scale);
    }
}
...
static void R_RenderSegLoop (int rw_x)
{
    ...
    // The original renderer selected one map per wall segment. Preserve that
    // zero-overhead behavior whenever depth fog is disabled.
    if (segtextured && !depthFog)
        dcvars.colormap = R_LoadColorMapPointer(R_ColourMapAdjusted(adjustedLight, 0, false));
    ...
    for ( ; rw_x < rw_stopx ; rw_x++)
    {
        ...
        if (segtextured)
        {
            ...
            // Fog uses 16 lighting bands and reuses each map for a two-column
            // pair. Classic mode already selected one map before the loop.
            if (depthFog && (!(rw_x & 1) || dcvars.colormap == colormaps))
                dcvars.colormap = R_LoadColorMapPointer(R_ColourMapAdjusted(adjustedLight, rw_scale, true));

            dcvars.iscale = R_FastReciprocal((unsigned)rw_scale);
        }
...
//performance profile.
// The old implementation stored node and side as two 32-bit values in a
// 128-entry array, so its effective maximum depth was 64 nodes. Pack side into
// bit 0 to preserve that limit while cutting the renderer stack by 256 bytes.
#define MAX_BSP_DEPTH 64

static void R_RenderBSPNode(int bspnum)
{
    unsigned int stack[MAX_BSP_DEPTH];
    int sp = 0;
    ...
    while(true)
    {
        ...
        while (!R_RenderBspSubsector(bspnum))
        {
            ...
            stack[sp++] = ((unsigned int)bspnum << 1) | (unsigned int)side;
...
// R_RenderView
//
// This function only orchestrates the frame; keeping the wrapper size-focused
// prevents O2 from cloning/inlining the entire renderer into a second copy.
#if defined(GBA)
