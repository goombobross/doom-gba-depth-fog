...
typedef struct gba_save_settings_t
{
    ...
    unsigned int depthFog;
    unsigned int extensionCookie;

} gba_save_settings_t;

const unsigned int settings_cookie = 0xbaddead1;
const unsigned int settings_depth_fog_cookie = 0xd3f09a11;
...
void G_SaveSettings()
{
    ...
    settings.depthFog = _g->depthFog;
    settings.extensionCookie = settings_depth_fog_cookie;
...
void G_LoadSettings()
{
    ...
    if(settings.cookie == settings_cookie)
    {
        ...
        // Preserve legacy settings while safely defaulting the new field on.
        if (settings.extensionCookie == settings_depth_fog_cookie && settings.depthFog <= 1)
            _g->depthFog = settings.depthFog;
        else
            _g->depthFog = 1;
