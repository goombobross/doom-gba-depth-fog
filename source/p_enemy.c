...
static boolean PIT_AvoidDropoff(const line_t *line)
{
            ...
            P_BoxOnLineSide(_g->tmbbox, line) == -1)
    {
                ...
                else
        {
                ...
                dist = FixedDistanceToTicks(dist, mobjinfo[actor->type].speed);
    ...
    A_Chase(actor, NULL);  // Return to normal attack.
}
...
void A_SkullAttack(mobj_t *actor, void*)
{
    ...
    dist = P_AproxDistance(dest->x - actor->x, dest->y - actor->y);
    dist = FixedDistanceToTicks(dist, SKULLSPEED);
...
void A_BrainSpit(mobj_t *mo, void*)
{
    ...
    if (_g->brain.targeton >= _g->numbraintargets)
        _g->brain.targeton = 0;                    // increment-and-wrap, no divide
...
void A_Detonate(mobj_t *mo, void*)
{
...
void A_Mushroom(mobj_t *actor, void*)
{
    int i, j, n = mobjinfo[actor->type].damage;

    A_Explode(actor, NULL);  // First make normal explosion
