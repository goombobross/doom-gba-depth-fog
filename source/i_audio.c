...
// Maxmod custom GBA configuration.
// MM_MIX_10KHZ is the hardware-supported 10,512 Hz output mode.
#define MAXMOD_CHANNEL_COUNT 12u

// The software mixer reads this buffer constantly, so keep it in IWRAM.
// 10 kHz needs 704 bytes versus 1,056 bytes at the default 16 kHz rate.
static u8 maxmod_mixing_buffer[MM_MIXLEN_10KHZ]
    __attribute__((aligned(4)));

// Match mmInitDefault's proven allocation model: reserve one zeroed EWRAM
// block before Doom claims the remaining external RAM for its zone heap.
// The wave buffer is first, followed by module/active/mixing channel data.
#define MAXMOD_CHANNEL_STORAGE \
    (MAXMOD_CHANNEL_COUNT * \
     (MM_SIZEOF_MODCH + MM_SIZEOF_ACTCH + MM_SIZEOF_MIXCH))
#define MAXMOD_WORK_BYTES (MM_MIXLEN_10KHZ + MAXMOD_CHANNEL_STORAGE)

static u8* maxmod_work_memory;
...
void I_InitSound(void)
{
...
#ifdef GBA
    // Zero-initialize the whole structure. Older Maxmod builds could read
    // padding beside mixing_mode as part of a word, so leaving stack garbage
    // here could cause intermittent startup/runtime failures.
    mm_gba_system system = {0};

    // mmInitDefault used calloc internally. Keep that behavior so the
    // active/effect channel state starts clean and the allocation is reserved
    // from EWRAM before Doom builds its zone heap.
    if (!maxmod_work_memory)
        maxmod_work_memory = (u8*)calloc(1, MAXMOD_WORK_BYTES);

    if (!maxmod_work_memory)
        I_Error("I_InitSound: unable to reserve Maxmod EWRAM");

    u8* const wave_channels = maxmod_work_memory;
    u8* const module_channels = wave_channels + MM_MIXLEN_10KHZ;
    u8* const active_channels = module_channels
        + MAXMOD_CHANNEL_COUNT * MM_SIZEOF_MODCH;
    u8* const mixing_channels = active_channels
        + MAXMOD_CHANNEL_COUNT * MM_SIZEOF_ACTCH;

    system.mixing_mode       = MM_MIX_10KHZ;
    system.mod_channel_count = MAXMOD_CHANNEL_COUNT;
    system.mix_channel_count = MAXMOD_CHANNEL_COUNT;

    system.module_channels = (mm_addr)module_channels;
    system.active_channels = (mm_addr)active_channels;
    system.mixing_channels = (mm_addr)mixing_channels;
    system.mixing_memory = (mm_addr)maxmod_mixing_buffer;
    system.wave_memory = (mm_addr)wave_channels;
    system.soundbank = (mm_addr)soundbank_bin;
    ...
    lprintf(LO_INFO,"I_InitSound: Maxmod ready at 10512 Hz");
}
