...
{
#if defined(GBA) && defined(NDEBUG)
    // Release GBA builds must not drag formatted console output into the hot path.
    (void)pri;
    (void)s;
    return 0;
#else
    ...
    va_list v;
    va_start(v, s);
    const int len = vsnprintf(msg, MAX_MESSAGE_SIZE, s, v);
    va_end(v);
