...
void P_UpdateSpecials (void)
{
    ...
    // Animate flats and textures globally.  The selected frame only changes
    // once every eight tics, so avoid recalculating and rewriting the tables
    // on the seven intervening tics.  leveltime starts at zero, which keeps
    // the initial mapping behavior identical.
    if ((_g->leveltime & 7) == 0)
    {
        const unsigned int t = (unsigned int)_g->leveltime >> 3;

        for (anim = _g->anims ; anim < _g->lastanim ; anim++)
        {
            const unsigned int n = anim->numpics;
