...
// Exact damage-thrust scaling without a variable software divide for every
// normal actor mass used by the built-in mobjinfo table.  The small remainder
// tables retain the exact truncation of (damage * 8192) / {5,6,10}.
inline static fixed_t P_DamageThrust(int damage, int mass)
{
  const unsigned int d = (unsigned int)damage;
  const fixed_t base = damage * (FRACUNIT >> 3);

  // Above 2621 damage the original multiply-by-100 expression can overflow
  // signed 32-bit arithmetic.  Keep that unusual path (for example telefrags)
  // out of the optimized switch.
  if (d <= 2621u)
  {
    switch (mass)
    {
      ...
      case 500:
      {
        static const unsigned short remainder5[5] =
          { 0, 1638, 3276, 4915, 6553 };
        const unsigned int q = (d * 52429u) >> 18;
        return (fixed_t)((q << 13) + remainder5[d - q * 5u]);
      }

      case 600:
      {
        static const unsigned short remainder6[6] =
          { 0, 1365, 2730, 4096, 5461, 6826 };
        const unsigned int q = (d * 43691u) >> 18;
        return (fixed_t)((q << 13) + remainder6[d - q * 6u]);
      }

      case 1000:
      {
        static const unsigned short remainder10[10] =
          { 0, 819, 1638, 2457, 3276, 4096, 4915, 5734, 6553, 7372 };
        const unsigned int q = (d * 52429u) >> 19;
        return (fixed_t)((q << 13) + remainder10[d - q * 10u]);
      }
    }
  }

  return base * 100 / mass;
}
...
void P_DamageMobj(mobj_t *target,mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  if (inflictor && !(target->flags & MF_NOCLIP) &&
       ...
       P_MobjIsPlayer(source)->readyweapon != wp_chainsaw))
    {
      ...
      fixed_t thrust = P_DamageThrust(damage, mobjinfo[target->type].mass);
